...
include('Base de Datos/Conexion/conexion.php');
  ...
  echo "<script>
  var resultado = window.confirm('¿Estas seguro que deseas eliminar el cliente ".$clave_Pro."? Recuerda que no se podra recuperar la información');
  if (resultado === true) {
  ...
  echo $SQL="DELETE FROM `clientes` WHERE `clientes`.`ClienteID` = '$clave_Pro'";
  $ResPro=$Conexion->query($SQL);

  if ($ResPro==1) {
  	echo "<script>
			alert('Se elimino el cliente $clave_Pro con exito');
